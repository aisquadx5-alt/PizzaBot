...
export async function POST(req: Request) {
  try {
    ...
    if (!isLoggedIn) {
      ...
      dynamicDirectives += `\n- The customer is logged in and has verified phone number: ${phoneNumber}.\n- Since the customer is authenticated, you can fully process, finalize, and confirm their order. When confirming an order, you MUST generate a random 4-digit order ID prefixed with '#' (e.g., #8921), inform the customer of their Order ID and that it is pending admin approval, and conclude your final response with this exact phrase, word-for-word, free of any asterisks or formatting:\n"Thank you! Our admin will call you on your registered phone number shortly to verify this order. Your pizza will be prepared ONLY after phone verification."\n- When confirming an order, you MUST append a tag exactly like this at the very end of your final message: [ORDER: {Generate_Random_4_Digit_ID} | {Item_Name} | {Quantity} | {Total_Price}]. NEVER use the #4012 example. Generate a real random ID like #8921. Do not add any extra characters, asterisks, bolding, or markdown inside or around the brackets. Just output the clean tag exactly.`;
    }
